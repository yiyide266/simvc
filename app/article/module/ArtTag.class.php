 ...
 * @author    kimmy
 */
class ArtTag extends \simvc\lib\module\Module{
	/**
	 * declare table name in this module 
	 * @attribute  protected
	 * @type string
	 */
	protected $tab_name = 'art_tag';
	/**
	...
	protected $params_filter = array(
			't_id' => array( '/^[\d]+$/' ),
			't_name' => array( '/^[\x{4e00}-\x{9fa5}\w\W]{3,16}$/u' ),//this pattern meanning : accept unicode (\x{4e00}-\x{9fa5}) and \w and \W chat between 3 and 16 lenght
		);
	/**
	...
	public function addOne( $data ){
		$re = $this -> filter( array( 't_name' ), $data );
		//var_dump($re);
	...
	public function delOne( $data ){
		$data = intval($data);
		return $this -> where(array( 't_id' => $data ) ) -> delete();
	}
	/**
	 * delete a tag
	 * @param  int   $data   r_id

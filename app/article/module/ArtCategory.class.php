 ...
 * @author    kimmy
 */
class ArtCategory extends \simvc\lib\module\Module{
	/**
	 * declare table name in this module 
	 * @attribute  protected
	 * @type string
	 */
	protected $tab_name = 'art_category';
	/**
	...
	/**
	 * call 'insert_art_category' procedures to insert
	...
	public function addOne( $data ){
		...
		$sql = sprintf( 'call insert_art_category(\'%s\', \'%s\', \'%d\', \'%d\', \'%d\')',$data['c_name'], $data['c_spec'], $data['c_type'], $data['c_pid'], $data['c_t_s'] );
		$re = $this -> getRow( $sql );
	...
	public function delOne( $data ){
		...
		if( $re[0]!=3 ){ return $re[0]==0?array(0):array(0, $re[1]); }
		//if( filter_sql_array( $data ) ){ return array(0); }
		$sql = sprintf( 'call drop_art_category( \'%d\' )',$data['c_id']  );
		$re = $this -> getRow( $sql );
